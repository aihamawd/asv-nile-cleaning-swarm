...
The project implements a mixed fleet of 3–4 physical autonomous surface vessels for Nile surface cleaning. One vessel is a new build; 2–3 vessels are inherited platforms restored and retrofitted where feasible.

## Vessel architecture

Each vessel is expected to separate functions across three logical layers where hardware permits:
...
The exact electronics implementation may differ between legacy and new-build vessels, provided the external fleet interface and safety behavior remain compatible.
...
Loss of shore communication does not remove onboard local autonomy. The vessel should enter documented degraded behavior using onboard safety and mission logic; stale fleet tasks are only released after the defined heartbeat/timeout policy.
