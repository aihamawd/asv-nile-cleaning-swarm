...
|---|---|---|
| REQ-001 | The project shall support cooperative operation of a mixed fleet of 3–4 physical ASVs where technically feasible, including one new-build vessel and inherited vessels restored/retrofitted as justified. | Inspection + fleet demonstration |
...
| REQ-003 | The fleet manager shall issue mission-level assignments and support reassignment of unfinished work when a vessel becomes unavailable or reaches an operational limit. | Multi-vessel functional test |
...
| REQ-005 | At least one vessel shall physically demonstrate floating-waste collection. | Collection experiment |
| REQ-006 | At least one vessel shall physically demonstrate a water-hyacinth cutting/handling subsystem selected through calculation and prototype evidence. | Cutter/handling experiment |
...
| REQ-012 | Final payload, vessel dimensions, hull material, cutter topology, propulsion hardware, advanced sensing, and task-allocation algorithm shall remain open until justified by engineering evidence or approved constraints. | Design-review audit |

## Notes

These are project-level requirements, not final acceptance thresholds. Numeric performance thresholds should be introduced only after legacy-vessel characterization, site/user constraints, and subsystem capability are measured.
